...
class DataGenerator(ABC):
    ...
    def generate_data(self, **kwargs) -> Tuple[pd.DataFrame, np.ndarray, np.ndarray, np.ndarray, List[str]]:
        ...
        Returns:
            X_df (pd.DataFrame): 特徴量データフレーム
            T (np.ndarray): 処置データ
            Y (np.ndarray): 観測結果
            true_cate (np.ndarray): 真のCATE値
...
class SyntheticDataGenerator(DataGenerator):
    ...
    def generate_data(
        ...
        Y_continuous = Y0 * (1 - T) + Y1 * T
        
        # バイナリ結果に変換（低い変換率を作るため）
        # シグモイド関数を使って確率に変換し、その後バイナリにサンプリング
        probs = 1 / (1 + np.exp(-Y_continuous + 2))  # +2でより低い変換率を作る
...
class CriteoDataGenerator(DataGenerator):
    ...
    def __init__(self, data_path: str = None, random_seed: int = 123):
        ...
        if data_path is None:
            # Default path to Criteo dataset (same directory as this file)
            current_dir = os.path.dirname(os.path.abspath(__file__))
            self.data_path = os.path.join(current_dir, "criteo-research-uplift-v2.1.csv.gz")
        else:
    ...
    def generate_data(
        ...
        Returns:
            ...
            T (np.ndarray): 処置データ
            Y (np.ndarray): 観測結果
            true_cate (np.ndarray): 推定されたCATE値
            feature_names (List[str]): 特徴量名リスト
        """
        ...
        # CATE値の推定（簡単なモデルベース推定）
        try:
            from sklearn.ensemble import RandomForestRegressor
            
            # 処置群と対照群での結果を使ってCATE推定
            control_mask = T == 0
            treatment_mask = T == 1
            
            if np.sum(control_mask) > 10 and np.sum(treatment_mask) > 10:
                # ランダムフォレストを使った簡単なCATE推定
                rf_control = RandomForestRegressor(n_estimators=50, random_state=self.random_seed)
                rf_treatment = RandomForestRegressor(n_estimators=50, random_state=self.random_seed)
                ...
                if np.sum(control_mask) > 0:
                    rf_control.fit(X_scaled[control_mask], Y[control_mask])
                    mu0 = rf_control.predict(X_scaled)
                else:
                    mu0 = np.full(len(Y), Y.mean())
                
                # 処置群モデル
                if np.sum(treatment_mask) > 0:
                    rf_treatment.fit(X_scaled[treatment_mask], Y[treatment_mask])
                    mu1 = rf_treatment.predict(X_scaled)
                else:
                    mu1 = np.full(len(Y), Y.mean())
                
                # CATE = E[Y|T=1, X] - E[Y|T=0, X]
                true_cate = mu1 - mu0
            else:
                # データが少ない場合は単純な差分を使用
                true_cate = np.full(len(Y), 0.0)
                
        except Exception as e:
            print(f"Warning: CATE estimation failed: {e}")
            # フォールバック: ランダムなCATE値
            true_cate = np.random.normal(0.0, 0.1, len(Y))
...
def generate_synthetic_data(n_samples=1000, random_seed=123, reward_type="linear", df=None):
    ...
    X_df, T, Y, true_cate, feature_names = generator.generate_data(n_samples=n_samples, reward_type=reward_type)
...
if __name__ == '__main__':
    ...
    X_df_syn, T_syn, Y_syn, true_cate_syn, feature_names_syn = synthetic_gen.generate_data(n_samples=10, reward_type="linear")
    print("Generated synthetic data - X_df shape:", X_df_syn.shape)
    print("Generated synthetic data - Y sample:", Y_syn[:5])
    print("Generated synthetic data - T sample:", T_syn[:5])
    print("Generated synthetic data - true_cate sample:", true_cate_syn[:5])
    print("Generated synthetic data - feature_names:", feature_names_syn)
